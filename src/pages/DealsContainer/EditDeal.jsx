...
import { useDealAssignMutation, useEditDealMutation } from "@/redux-store/api/dealsApi";
...
const SelectField = ({ id, label, options, value, register, setValue, errors }) => (
   <div className="space-y-2">
      <label htmlFor={id} className="transition-all duration-300 text-base font-medium text-title peer-focus:text-blue-500">
         {label}
      </label>
      <Select {...register(id)} onValueChange={(value) => setValue(id, value)} value={value} >
...
const EditDeal = ({ isOpen, setOpen, dealDetails, refetch }) => {
   const [editDeal, { isLoading }] = useEditDealMutation();
   ...
   useEffect(() => {
      if (dealDetails) {
         reset({
            lender_name: dealDetails?.lender_name || "",
            security_property_add: dealDetails?.security_property_add || "",
            loan_amount: dealDetails?.loan_amount || "",
            deal_type: dealDetails?.deal_type || "",
            deal_stage: dealDetails?.deal_stage || "",
         });
         setValue("deal_stage", dealDetails?.deal_stage || "");
         setSelectedMembers(dealDetails?.assigned_members || []);
         setSelectedClients(dealDetails?.clients || []);
      }
   }, [dealDetails, reset, setValue]);

   return (
      <CmModal isOpen={isOpen} handleClose={handleClose} size="800px" title="Edit Deal">
